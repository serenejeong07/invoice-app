...
if st.session_state.extracted_data is not None:
    ...
    if "error" in extracted_data:
        ...
        with col1:
            ...
            # 좌측 패널 정보는 건드리지 않고 원문 그대로 유지
            e_consignee_name = st.text_input("수취인 이름", value=extracted_data.get("consignee_name", ""))
            e_consignee_phone = st.text_input("수취인 전화번호", value=extracted_data.get("consignee_phone", ""))
            e_consignee_address = st.text_input("수취인 상세주소", value=extracted_data.get("consignee_address", ""))
            e_consignee_country = st.text_input("수취인 나라", value=extracted_data.get("consignee_country", ""))
            e_consignee_zipcode = st.text_input("수취인 우편번호", value=extracted_data.get("consignee_zipcode", ""))
        ...
        if not raw_items:
            df_items = pd.DataFrame([{"name": "", "quantity": ""}])
        else:
